...
export default function ListingDetailScreen() {
  ...
  const handleReport = () => {
    Alert.alert('İlan Sahibi Hakkında', 'Ne yapmak istiyorsunuz?', [
      {
        ...
        onPress: () => {
          Alert.alert('Engelle', 'Bu kullanıcının ilanları ve talepleri artık size gösterilmeyecek.', [
            { text: 'Vazgeç', style: 'cancel' },
            {
              ...
              style: 'destructive',
              onPress: async () => {
                const { error } = await blockUser(currentUserId, listing.agent_id);
      ...
      },
      {
        ...
        onPress: () => {
          ...
          Alert.alert('İlanı Bildir', 'Bildirme nedeninizi seçin:', reasons.map((r) =>
            r.style ? { text: r.text, style: r.style } : {
              ...
              onPress: async () => {
                ...
                Alert.alert(error ? 'Hata' : 'Bildirildi', error ?? 'İlan incelenmek üzere bildirildi. Teşekkürler.');
              },
            }
          ));
        },
  ...
  return (
    <>
      ...
      >
        ...
        {/* Kira Kontratı — sadece kiralık ilanlarda, ilan sahibi görsün */}
        {isOwner && !isSale && (
          <Pressable
            style={({ pressed }) => [styles.rentalContractButton, pressed && { opacity: 0.85 }]}
            onPress={() => {
...
const styles = StyleSheet.create({
  ...
  rentalContractButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.rent + '0A', borderRadius: Radius.lg, padding: Spacing.lg, gap: Spacing.sm, marginBottom: Spacing.lg, borderWidth: 1, borderColor: Colors.rent + '28' },
  rentalContractButtonText: { ...Typography.subhead, color: Colors.rent, fontWeight: '600', flex: 1 },
});
